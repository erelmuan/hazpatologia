    ...
    echo Form::widget([ // fields with labels
      //  'contentBefore'=>'<legend class="text-info"><small>Datos del paciente</small></legend>',
        ...
        'form'=>$form,
         'columns'=>5,
         'attributes'=>[
         'Paciente'=>['label'=> "Paciente" ,'options'=>['value'=>$model->pap->solicitudpap->paciente->apellido." ". $model->pap->solicitudpap->paciente->nombre ,'readonly'=> true ,'url' => '#' ],'columnOptions'=>['class'=>'col-lg-3',],],
           'DNI'=>['label'=>'DNI', 'options'=>['value'=>$model->pap->solicitudpap->paciente->num_documento, 'placeholder'=>'documento...','readonly'=> true],'columnOptions'=>['class'=>'col-sm-2']],
           'Fecha_nacimiento'=>['label'=>'Fecha de nac.', 'options'=>['value'=> date("d/m/Y",strtotime("'".$model->pap->solicitudpap->paciente->fecha_nacimiento."'")), 'placeholder'=>'Fecha de nacimiento...','readonly'=> true],'columnOptions'=>['class'=>'col-sm-2']],

           'Edad'=>['label'=>'Edad', 'options'=>['value'=>$model->pap->solicitudpap->calcular_edad(),  'placeholder'=>'Edad...','readonly'=> true],'columnOptions'=>['class'=>'col-sm-1']],
...
<div class="vph-escaneada-form">
    ...
    <div class="col-sm-6 col-sm-8 col-dm-9 form-group">
      ...
      <p>
          <? if (isset($model->documento )&& !empty($model->documento)){ ?>
            <iframe src=<?=Url::base().'/uploads/vph/'. str_replace("+", "%20",urlencode($model->documento)) ?> height="400" width="300"></iframe>
          <? }else {
    ...
    <?= $form->field($model, 'id_pap')->hiddenInput()->label(false); ?>
    <?php if (!Yii::$app->request->isAjax){ ?>
        <div class="form-group">
            <?= Html::submitButton($model->isNewRecord ? 'Crear' : 'Actualizar', ['class' => $model->isNewRecord ? 'btn btn-success' : 'btn btn-primary']) ?>
            <? if (!$model->isNewRecord){
               echo  Html::a('<i class="glyphicon glyphicon-arrow-right"></i> Ir los informes',['/pap/view', 'id'=>$model->pap->id], ['class'=>'btn btn-success grid-button']);
            }
            ?>
